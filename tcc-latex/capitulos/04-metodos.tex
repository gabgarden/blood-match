...
Neste capítulo são descritos os procedimentos metodológicos adotados para o alcance dos objetivos propostos, abordando a abordagem de pesquisa escolhida, os instrumentos utilizados na coleta e análise de informações, as ferramentas empregadas no desenvolvimento da API e o cronograma de execução do trabalho.
...
\begin{citacao}
A pesquisa qualitativa trabalha com o universo de significados, motivos, aspirações, crenças, valores e atitudes, o que corresponde a um espaço mais profundo das relações, dos processos e dos fenômenos que não pode ser reduzido à operacionalização de variáveis. \citeonline[p.~21]{minayo2001}
\end{citacao}
...
Por fim, os resultados foram avaliados por meio de testes unitários, seguidos da elaboração das conclusões do trabalho.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}X c c c c c c}
    \toprule
    \textbf{Atividade} & \textbf{Ago/25} & \textbf{Set/25} & \textbf{Out/25} & \textbf{Nov/25} & \textbf{Dez/25} & \textbf{Jan/26} \\
    \midrule
    Definição do escopo, problema e objetivos & X & X &  &  &  &  \\
    Fundamentação teórica (OO, SOLID, DDD) &  & X & X & X &  &  \\
    Pesquisa documental do domínio hemoterápico &  & X & X &  &  &  \\
    Protocolo e mapeamento sistemático da literatura &  &  & X & X & X &  \\
    Análise de domínio com DDD &  &  &  & X & X & X \\
    Levantamento de requisitos (RF/RNF) &  &  &  &  & X & X \\
    Redação dos Capítulos 1 a 4 & X & X & X & X & X & X \\
    \bottomrule
  \end{tabularx}
  \fonte{Elaborado pelos próprios autores (2025).}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}X c c c c c c}
    \toprule
    \textbf{Atividade} & \textbf{Fev/26} & \textbf{Mar/26} & \textbf{Abr/26} & \textbf{Mai/26} & \textbf{Jun/26} & \textbf{Jul/26} \\
    \midrule
    Modelagem UML (casos de uso e classes) & X & X &  &  &  &  \\
    Implementação da API (Java / Spring Boot) &  & X & X & X &  &  \\
    Persistência (MongoDB) e autenticação (JWT) &  &  & X & X &  &  \\
    Motor de recomendação de doadores &  &  & X & X & X &  \\
    Testes unitários e de integração &  &  &  & X & X &  \\
    Avaliação da proposta e análise de resultados &  &  &  &  & X & X \\
    Redação final (Capítulos 5 a 7) e revisão & X & X & X & X & X & X \\
    \bottomrule
  \end{tabularx}
  \fonte{Elaborado pelos próprios autores (2025).}
\end{table}
