...
\begin{itemize}
  ...
  \item \textbf{Isolamento da Camada Web via DTOs Imutáveis (\textit{Records}):} Os objetos \codigo{Input} e \codigo{Output} são definidos como Java \textit{records}, garantindo imutabilidade e eliminação de efeitos colaterais. O \codigo{Input} não possui acoplamento com anotações de infraestrutura HTTP (como \codigo{@Valid} ou anotações do Jackson), sendo a camada de apresentação responsável por mapear o DTO da requisição para o \codigo{Input}.
...
public class DonationRequest extends DomainObject {
  ...
  // Metodo Fabril Estatico (Static Factory Method)
...
\subsection{Camada de Infraestrutura: Repositório e Controle de Concorrência Otimista}

Na extremidade da arquitetura, a camada de infraestrutura traduz o modelo conceitual de domínio para as estruturas do banco de dados (MongoDB). Para proteger o sistema contra acessos simultâneos concorrentes (como a alteração concorrente da meta e do prazo de uma mesma solicitação), adota-se formalmente o mecanismo de \textbf{Controle de Concorrência Otimista} (\textit{Optimistic Concurrency Control} --- OCC), fundamentado nas proposições clássicas de Kung e Robinson (1981) e Fowler (2002). No esquema de persistência, o OCC é suportado pela anotação \codigo{@Version}, conforme exibido na Listagem~\ref{lst:donation-request-schema}.
