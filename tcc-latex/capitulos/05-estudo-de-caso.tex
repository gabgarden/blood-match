...
A representação gráfica dessas interações é dividida de acordo com o nível de acesso dos atores. A Figura~\ref{fig:uc-guest} apresenta o diagrama de caso de uso do ator (\textit{Guest}).

\begin{figure}[H]
  \centering
  \caption{Diagrama de caso de uso de visitante}
  \includegraphics[width=0.6\textwidth]{figura03-uc-visitante}
  \fonte{Elaborado pelos próprios autores (2026).}
  \label{fig:uc-guest}
\end{figure}
...
O ator \textit{Requester} concentra as permissões relacionadas à criação e à gestão de solicitações de sangue (UC09 e UC14), além do disparo de notificações a doadores potenciais quando uma solicitação precisa de maior alcance (UC15).
...
O ator \textit{Donor} possui acesso ao núcleo do processo de doação, incluindo a consulta às solicitações recomendadas, o registro unificado de doações, a conclusão e o reagendamento de pendentes, a edição de seu perfil e preferências de recomendação, a gestão do próprio histórico hemoterápico e a consulta ao seu resumo (UC08, UC10, UC11, UC12, UC13, UC16 e UC17). No UC11, a doação nasce pendente quando o doador informa a data de intenção ou concluída quando informa a data efetiva --- um único caso de uso, sem fábricas concorrentes no domínio.
...
O controle de integridade transacional contra atualizações concorrentes foi estabelecido de maneira uniforme em toda a camada de persistência e exposto às camadas externas: todos os \textit{schemas} que mapeiam os agregados e entidades do sistema (como \texttt{DonationRequestSchema}, \texttt{DonationSchema}, \texttt{BloodCenterScheduleSchema} e \texttt{BloodCenterInventorySchema}) incorporam o controle de versão otimista por meio do atributo \codigo{version} anotado com \codigo{@Version}. Para garantir uma integridade de ponta a ponta, todos os casos de uso de atualização (\textit{PATCH}) e de exclusão (\textit{DELETE}) exigem explicitamente que o \textit{Frontend} informe a versão original (\codigo{version}) na qual o usuário se baseou para a modificação. O caso de uso carrega a entidade do repositório, valida se a versão informada coincide com a versão salva e só então efetua a alteração. Desse modo, escritas concorrentes sobre qualquer documento --- a exemplo da alteração simultânea do prazo e da meta de bolsas em uma solicitação por usuários diferentes em telas distintas --- são prontamente detectadas pela aplicação antes mesmo da gravação final.

Para assegurar o desacoplamento arquitetural no momento da persistência, todas as operações de escrita nos adaptadores de repositório são encapsuladas pelo componente utilitário \codigo{OptimisticConcurrency}. Ao identificar um eventual conflito de versão subjacente não capturado pelas checagens lógicas da aplicação, o componente captura a \codigo{OptimisticLockingFailureException} do \textit{Spring Data} e a converte em uma \codigo{ConcurrencyException} (exceção da camada de aplicação derivada de \codigo{ApplicationException}), sinalizando de forma explícita ao caso de uso que a entidade foi alterada por outra transação e precisa ser recarregada antes de uma nova tentativa --- impedindo, novamente, que exceções específicas da tecnologia de persistência alcancem as camadas superiores. O cumprimento de metas, descrito na Seção~\ref{sec:fulfillment-bags}, não participa desse conflito: o \textit{snapshot} é calculado em memória e não regrava a solicitação. Além disso, índices compostos foram definidos sobre situação ativa, tipo sanguíneo e data-limite, e sobre situação ativa, organização e data da solicitação --- este último alinhado à ordenação exigida pela alocação, evitando uma ordenação adicional em memória ---, bem como um índice geoespacial \texttt{2dsphere} sobre a localização do hemocentro, otimizando as consultas de recomendação descritas na Seção~\ref{sec:recomendacao-fulfillment}.
