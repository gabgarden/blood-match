...
Neste nível de abstração, evidencia-se a associação semântica entre a solicitação (\texttt{DonationRequest}) e o ato de doar (\texttt{Donation}). O fluxo natural demonstra que um \texttt{Requester} origina a solicitação, que é vinculada a um \texttt{BloodCenter} e, posteriormente, recebe as doações efetivadas por um \texttt{Donor}.
...
A notificação de doadores é abstraída por uma interface de domínio (\texttt{NotificationServiceInterface}), permitindo múltiplas implementações na camada de infraestrutura --- como uma implementação por e-mail e uma implementação simplificada para ambiente de desenvolvimento --- sem que o caso de uso que dispara a notificação precise conhecer o mecanismo concreto de envio. Essa inversão de dependência, alinhada aos princípios SOLID discutidos no Capítulo~2, evita que o domínio fique acoplado a bibliotecas de terceiros.
...
O caminho de leitura é implementado pelo \texttt{GetRecommendedRequestsUseCase}, acionado pelo \textit{endpoint} \texttt{GET /donation-requests/recommendations}. Dado um doador autenticado, o caso de uso primeiro verifica sua elegibilidade para doar na data corrente (\texttt{Donor.isEligibleToDonate(...)}), considerando faixa etária e intervalo mínimo desde a última doação. Em seguida, busca no repositório as solicitações ativas compatíveis com o tipo sanguíneo do doador, dentro da distância máxima de recomendação configurada por ele (RF22) e ainda não expiradas, aproveitando o índice geoespacial descrito na Seção anterior.

As solicitações candidatas que já atingiram a meta (\texttt{isGoalReached()}) são descartadas, e as demais são ordenadas por proximidade geográfica, urgência e proximidade da data-limite, de modo que o doador visualize primeiro as solicitações mais próximas, mais urgentes e mais próximas de expirar. O resultado retornado inclui, para cada solicitação recomendada, a distância estimada, a urgência, a meta de bolsas e o total já cumprido --- este último lido diretamente do campo persistido \texttt{fulfilledBloodBags}, sem qualquer recálculo nesse momento.

\subsection{Notificação de doadores potenciais}

Complementarmente à recomendação, o requisitante pode disparar notificações ativas por meio do \textit{endpoint} \texttt{POST /donation-requests/\{id\}/notify}, implementado pelo \texttt{NotifyPotentialDonorsUseCase}. Esse caso de uso valida que o solicitante autenticado é o dono da solicitação, que a solicitação ainda não expirou e que sua meta ainda não foi atingida; em seguida, delega ao \texttt{FindEligibleDonorsUseCase} (por meio do serviço de domínio \texttt{DonorMatchingService}) a identificação dos doadores elegíveis --- considerando compatibilidade sanguínea e elegibilidade temporal --- para, então, notificar cada doador elegível com conta ativa através do serviço de notificação descrito na Seção~5.5.3. Esse fluxo materializa, na arquitetura da API, o comportamento observado na literatura (Capítulo~3) de notificar diretamente candidatos compatíveis em vez de aguardar passivamente novas doações.

\subsection{Materialização do cumprimento de metas (\texttt{fulfilledBloodBags})}

Não existe um vínculo direto entre uma doação e uma solicitação específica: cada doação concluída em um hemocentro entra em um agrupamento (\textit{pool}) de doações daquela organização e é distribuída entre as solicitações ativas do mesmo hemocentro pelo \texttt{DonationRequestFulfillmentService}, seguindo uma regra de alocação \textit{First In, First Out} (FIFO) por hemocentro: as solicitações são ordenadas da mais antiga para a mais nova e, para cada doação (também ordenada cronologicamente), o serviço percorre as solicitações compatíveis --- mesmo tipo sanguíneo, dentro da janela entre a data de solicitação e a data-limite, e com meta ainda não atingida --- alocando a doação à primeira solicitação elegível. A verificação de compatibilidade entre uma doação e uma solicitação específica é encapsulada no método \texttt{DonationRequest.acceptsDonation(...)}, preservando a regra de negócio dentro do agregado.

Diferentemente da estratégia inicialmente cogitada de recalcular essa alocação a cada leitura, optou-se por materializar o resultado no momento da escrita: o campo \texttt{fulfilledBloodBags} é persistido na própria solicitação e recalculado apenas quando o conjunto de doações concluídas de um hemocentro é alterado. Esse recálculo é disparado pelo \texttt{DonationRequestFulfillmentRefresher} a partir de dois casos de uso --- \texttt{CreateCompletedDonationUseCase} e \texttt{CompletePendingDonationUseCase} ---, que, após persistirem a doação, carregam as solicitações ativas e as doações concluídas da organização afetada, executam o método \texttt{synchronize(...)} do serviço de \textit{fulfillment} e persistem cada solicitação com o novo valor calculado.

Essa abordagem caracteriza um contador materializado (\textit{materialized/denormalized counter}): as leituras de recomendação e de listagem de solicitações por participante apenas consultam o campo já calculado, enquanto o custo do algoritmo de alocação é pago somente nas escritas que efetivamente alteram o cenário de doações concluídas --- criação de solicitação, cancelamento e alterações de meta ou prazo não disparam o recálculo, permanecendo consistentes até a próxima doação concluída daquele hemocentro. Trata-se de uma decisão de desempenho deliberada: como as leituras (recomendações e listagens) tendem a ser bem mais frequentes do que a conclusão de doações, deslocar o custo computacional do caminho de leitura para o caminho de escrita reduz a carga da API sem comprometer a corretude do algoritmo FIFO, que continua sendo recalculado por completo --- e não incrementado ingenuamente --- a cada sincronização.
