...
Um ponto de destaque é o controle de concorrência da solicitação de doação: o \texttt{DonationRequestSchema} utiliza controle de versão otimista (anotação \texttt{@Version}), de forma que duas escritas concorrentes sobre a mesma solicitação --- por exemplo, a atualização do prazo e a da meta de bolsas --- sejam detectadas pelo MongoDB. Ao identificar um conflito de versão, o repositório converte a exceção de persistência em uma \texttt{IllegalStateException}, sinalizando de forma explícita ao caso de uso que a operação deve ser reavaliada. O cumprimento de metas, descrito na Seção~\ref{sec:fulfillment-bags}, não participa desse conflito: o \textit{snapshot} FIFO é calculado em memória e não regrava a solicitação. Além disso, índices compostos foram definidos sobre campos como situação ativa, tipo sanguíneo e data-limite, bem como um índice geoespacial sobre a localização do hemocentro, otimizando as consultas de recomendação descritas na Seção~\ref{sec:recomendacao-fulfillment}.
...
Não existe vínculo direto entre uma doação e uma solicitação específica: cada doação concluída em um hemocentro entra em um agrupamento (\textit{pool}) de doações daquela organização e é distribuída entre as solicitações ativas do mesmo hemocentro pelo \codigo{DonationRequestFulfillmentService} (Listagem~\ref{lst:fifo}). Os casos de uso de leitura invocam \codigo{fill} com a data de referência \codigo{asOfDate} --- em geral a data corrente. O serviço recarrega as solicitações ativas e ainda não expiradas dos hemocentros envolvidos (\codigo{findActiveRequestsByOrganizationIds}), de modo que o FIFO considere o pool inteiro do centro, e não apenas o recorte da recomendação ou da listagem do requisitante. As doações concluídas são lidas na janela \codigo{[data da solicitação ativa mais antiga, asOfDate]}. A alocação ocorre em memória, isolada por hemocentro, em regra \textit{First In, First Out} (FIFO): as solicitações são ordenadas da mais antiga para a mais nova e, para cada doação (ordenada pela data efetiva), percorre as solicitações que ainda têm meta e que \codigo{acceptsDonation} aceita --- ativa, não expirada na data de referência, doação concluída, data efetiva na janela \codigo{[dateRequested, dateLimit]} e tipo sanguíneo do doador compatível via \codigo{canDonateTo} (Listagem~\ref{lst:accepts-donation}) --- alocando a bolsa à primeira elegível. O \texttt{break} garante uma bolsa por doação.
