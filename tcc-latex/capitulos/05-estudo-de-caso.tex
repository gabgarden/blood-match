...
Devido à abrangência da API e visando manter a concisão documental, optou-se por detalhar textualmente apenas os casos de uso críticos para o núcleo da aplicação, adotando a estrutura formal de especificação textual proposta por Cockburn (2001). Essa abordagem documenta o fluxo de eventos, as restrições operacionais e as pré-condições de segurança exigidas, justificando arquiteturalmente a ausência de relacionamentos visuais de dependência de autenticação nos diagramas UML.

\vspace{0.5cm}

% --- UC09 ---
\noindent\textbf{UC09: Registrar solicitação de doação}
\begin{description}
    \item[Ator Principal:] Requisitante (\textit{Requester})
    \item[Pré-condição:] O usuário deve estar autenticado, possuir um token JWT válido e estar registrado com o papel de Requisitante.
    
    \item[Cenário Principal:] \itemsep0em
    \begin{enumerate}
        \item O Requisitante acessa o sistema e solicita a criação de uma nova campanha.
        \item O Requisitante informa os dados da necessidade: tipo sanguíneo, quantidade de bolsas, data limite, nível de urgência e hemocentro responsável.
        \item O sistema valida as regras de negócio, assegurando a integridade dos invariantes.
        \item O sistema persiste a solicitação ativa.
        \item O sistema retorna a confirmação de criação com o identificador único gerado.
    \end{enumerate}
    
    \item[Cenários Alternativos:] \itemsep0em
    \begin{itemize}
        \item \textbf{3a. Falha de validação de domínio} (ex: data limite no passado ou meta nula):
        \begin{itemize}
            \item O sistema rejeita a operação.
            \item O sistema retorna a descrição estruturada do erro, abortando o fluxo.
        \end{itemize}
    \end{itemize}
\end{description}
...
% --- UC10 ---
\noindent\textbf{UC10: Consultar recomendações de solicitações}
\begin{description}
    \item[Ator Principal:] Doador (\textit{Donor})
    \item[Pré-condição:] O usuário deve estar autenticado, possuir um token JWT válido e estar registrado com o papel de Doador.
    
    \item[Cenário Principal:] \itemsep0em
    \begin{enumerate}
        \item O Doador solicita a visualização de campanhas recomendadas.
        \item O sistema verifica a elegibilidade temporal do doador com base em sua última doação registrada.
        \item O sistema busca as solicitações ativas e realiza o cruzamento de compatibilidade tipológica.
        \item O sistema aplica o filtro geoespacial baseado na distância máxima configurada pelo usuário.
        \item O sistema ordena os resultados priorizando urgência, proximidade e vencimento do prazo, devolvendo a listagem ao Doador.
    \end{enumerate}
    
    \item[Cenários Alternativos:] \itemsep0em
    \begin{itemize}
        \item \textbf{2a. Doador inelegível temporariamente:}
        \begin{itemize}
            \item O sistema interrompe o processamento heurístico.
            \item O sistema retorna uma lista vazia ou um indicativo de inaptidão.
        \end{itemize}
    \end{itemize}
\end{description}
...
% --- UC12 ---
\noindent\textbf{UC12: Completar doação pendente}
\begin{description}
    \item[Ator Principal:] Doador (\textit{Donor})
    \item[Pré-condição:] O usuário deve estar autenticado, possuir um token JWT válido e estar registrado com o papel de Doador.
    ...
    \item[Cenário Principal:] \itemsep0em
    \begin{enumerate}
        \item O Doador submete a data de efetivação para uma intenção de doação previamente registrada.
        \item O sistema recupera a doação pendente associada ao identificador.
        \item O sistema valida a consistência cronológica da data informada em relação à data atual.
        \item O sistema transiciona o estado do agregado para concluído.
        \item O sistema atualiza o registro do Doador para computar a nova janela de descanso regulamentar.
    \end{enumerate}
\end{description}
...
\subsection{Camada de Interface: REST Controller e DTOs}

A camada de interface atua como o ponto de entrada da aplicação, isolando o núcleo do sistema dos detalhes do protocolo HTTP. O \codigo{CreateDonationRequestController} é responsável por interceptar as requisições, executar validações sintáticas básicas e converter o \textit{payload} da requisição (DTO) para o modelo de entrada do caso de uso.
