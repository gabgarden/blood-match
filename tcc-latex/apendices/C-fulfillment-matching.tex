...
A Listagem~\ref{lst:fifo} sintetiza \codigo{calculate} e \codigo{distributeDonations} do \codigo{DonationRequestFulfillmentService}. Solicitações e doações são ordenadas cronologicamente; cada doação percorre as solicitações do mesmo hemocentro e é atribuída à primeira que a aceita (\codigo{acceptsDonation}) e ainda não atingiu a meta --- o \texttt{break} encerra a busca, concretizando a política FIFO.
